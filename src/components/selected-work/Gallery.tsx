...
export default function Gallery({
  ...
  return (
    <dialog
      ...
      className="gallery-dialog fixed inset-0 m-0 size-full max-h-none max-w-none overscroll-contain border-0 bg-transparent p-0 outline-none backdrop:bg-black/25 backdrop:backdrop-blur-[3px] dark:backdrop:bg-black/55"
      onClose={onClose}
      onClick={(event) => {
        if (event.currentTarget === event.target) closeDialog();
      }}
    >
      <div className="gallery-panel-enter absolute inset-0 isolate m-0 overflow-hidden border-[0.5px] border-text-primary/12 bg-surface/97 shadow-[0_24px_80px_-24px_rgb(17_17_15/0.38),0_2px_10px_rgb(17_17_15/0.08)] sm:m-4 sm:rounded-[18px] lg:m-14 lg:rounded-3xl dark:shadow-[0_28px_90px_-24px_rgb(0_0_0/0.72),0_2px_12px_rgb(0_0_0/0.32)]">
        <GalleryNavigation
          slides={images}
        ...
        {galleryLink && (
          <Link
            ...
            rel="noreferrer"
            className={cx(
              "group absolute active:scale-97 bottom-[max(0.80rem,env(safe-area-inset-bottom))] left-3 z-20 flex min-h-8 items-center rounded-full border-[0.5px] border-text-primary/12 bg-surface/92 px-3 text-[10px] font-semibold uppercase tracking-wider text-text-primary shadow-[0_2px_7px_rgb(17_17_15/0.05)] backdrop-blur-xl transition-[background-color,border-color] duration-150 hover:border-text-primary/22 hover:bg-text-primary/[0.025] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-project-accent sm:bottom-4 sm:left-4 dark:shadow-[0_2px_9px_rgb(0_0_0/0.18)] dark:hover:border-white/18 dark:hover:bg-white/[0.04]",
        ...
        <div className="absolute left-3 top-3 z-20 flex h-8 max-w-[calc(100%-4.75rem)] items-center gap-1.5 rounded-full border-[0.5px] border-text-primary/12 bg-surface/92 px-3 text-[10px] font-semibold uppercase tracking-wider text-text-primary shadow-[0_2px_7px_rgb(17_17_15/0.05)] backdrop-blur-xl sm:left-4 sm:top-4 sm:max-w-[calc(100%-5rem)] dark:shadow-[0_2px_9px_rgb(0_0_0/0.18)]">
          <span className="truncate">{project.name}</span>
          <span className="text-text-muted" aria-hidden="true">
            /
          </span>
          <span className="shrink-0 text-text-muted">Selected work</span>
        </div>
        ...
        >
          {images.map((item, index) => (
            ...
            >
              ...
              <div
                className="relative w-full shrink-0 overflow-hidden rounded-[10px] border-[0.5px] border-text-primary/12 bg-surface shadow-[0_18px_50px_-26px_rgb(17_17_15/0.38)] after:absolute after:top-1/2 after:z-10 after:hidden after:h-[0.5px] after:w-19 after:-right-19.5 after:bg-border-subtle sm:rounded-xl lg:after:block dark:shadow-[0_20px_56px_-24px_rgb(0_0_0/0.68)]"
                style={{
                  aspectRatio:
                    "aspectRatio" in item
                      ? item.aspectRatio
